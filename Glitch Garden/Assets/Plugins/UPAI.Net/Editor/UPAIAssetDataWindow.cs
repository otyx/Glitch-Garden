...
namespace RKGamesDev.Systems.UPAI.Editor {
  ...
  public class UPAIAssetDataWindow : UPAIBaseEditorWindow {
    private const string CLASS_NAME = "UPAIAssetDataWindow";
    private const float MAX_IMAGE_DIMENSION = 128f;

    private Document _assetDocument = null;
    private string[] _assetTypes = null;
    private bool _autoTriggerImport = false;
    ...
    private UPAIIndexableAssetDependencyPagingHelper _pagingHelper = null;
    private Vector2 _scrollPosition_MetaData;
    private Vector2 _scrollPosition_TagData;
    ...
    //private UPAIIndexableAssetDependencyPagingHelper _tempPagingHelper = null;
    private UPAIUINotificationsProcessor _uiNotificationProcessor = null;
    private UPAIIndexableAssetInfo _assetToImport = null;
    private UPAIAssetTypeDefintion _assetTypeDefintion = null;
    private bool _packageViewAllowed = false;
    private int _totalAssetDependencies = 0;
    ...
    /// 
    /// </summary>
    public static void CreateAssetDataWindow(Document assetDocument, bool allowViewPackage = true, bool autoTriggerImportAsset = false) {
      UPAISystem.Debug.LogDebugMessage("{0}.{1}()", CLASS_NAME, "CreateAssetDataWindow");

      if (assetDocument == null)
        return;

      var assetFileName = _GetField(assetDocument, UPAIDocumentFieldNames.ASSET_FILENAME_FIELDNAME);
      ...
      var newAssetDataWindow = (UPAIAssetDataWindow)EditorWindow.CreateInstance<UPAIAssetDataWindow>();
      newAssetDataWindow.titleContent = new GUIContent() {
        text = string.Format("{0} Data ({1})...", assetFileName, assetType)
      };
      if (!newAssetDataWindow.Initialize(assetDocument, allowViewPackage, autoTriggerImportAsset)) {
        newAssetDataWindow.Close();
        newAssetDataWindow = null;
        return;
      }

      try {
        UPAIUnityEditorRecoveryManager.AssetWindowOpened(
          assetDocument.GetField(UPAIDocumentFieldNames.ASSET_ID_FIELDNAME).StringValue);
      } catch {

      }

      newAssetDataWindow.minSize = new Vector2(805f, 650f);
      newAssetDataWindow.maxSize = new Vector2(805f, 650f);
      newAssetDataWindow.CenterOnMainWin();
      newAssetDataWindow.ShowUtility();
      newAssetDataWindow.Focus();
    }
    ...
    public UPAIAssetTypeEnum GetSelectedAssetType() {
      UPAISystem.Debug.LogDebugMessage("{0}.{1}()", CLASS_NAME, "GetSelectedAssetType");
    ...
    /// <param name="allowViewPackage"></param>
    /// <returns></returns>
    public bool Initialize(Document assetDocument, bool allowViewPackage = true, bool autoTriggerImportAsset = false) {
      UPAISystem.Debug.LogDebugMessage("{0}.{1}()", CLASS_NAME, "Intitialize");

      if (assetDocument == null)
        throw new ArgumentNullException("AssetDocument");
      ...
      try {
        ...
        _packageViewAllowed = allowViewPackage;

        _uiNotificationProcessor = new UPAIUINotificationsProcessor(
          new List<UPAIProcessTypeEnum>() {
          UPAIProcessTypeEnum.ExtractAsset,
          UPAIProcessTypeEnum.ImportAsset
          },
          null,
          null);
        
        _selectedAssetId = _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_ID_FIELDNAME).StringValue;

        try {
          _totalAssetDependencies = UPAIIndexSearcher.GetIndexableAssetDependencies(
            Application.unityVersion, _selectedAssetId).Count;
          _assetTypes = UPAIIndexSearcher.GetIndexableAssetDependencyTypes(
            Application.unityVersion, _selectedAssetId, true).Keys.OrderBy(x => x).ToArray();
          _ChangeAssetType(0);
        } catch {
          _totalAssetDependencies = 0;
          _assetTypes = null;
        }

        _autoTriggerImport = autoTriggerImportAsset;
    ...
    void OnEnable() {
      ...
      UPAISystem.AssetImportProcessAborted += UPAISystem_AssetImportProcessAborted;
      UPAISystem.AssetImportProcessAborting += UPAISystem_AssetImportProcessAborting;
      UPAISystem.AssetImportProcessCompleted += UPAISystem_AssetImportProcessCompleted;
      UPAISystem.AssetImportProcessCompleting += UPAISystem_AssetImportProcessCompleting;
      UPAISystem.AssetImportProcessStarted += UPAISystem_AssetImportProcessStarted;
      UPAISystem.AssetImportProcessStarting += UPAISystem_AssetImportProcessStarting;
    ...
    void OnDestroy() {
      ...
      try {
        UPAIUnityEditorRecoveryManager.AssetWindowClosed(
          _assetDocument.GetField(UPAIDocumentFieldNames.ASSET_ID_FIELDNAME).StringValue);
      } catch {
      ...
      _assetToImport = null;
      _assetTypeDefintion = null;
      _packageViewAllowed = false;
  }
    ...
    void _ChangeAssetType(int selectedAssetIndex) {
      ...
      _pagingHelper = new UPAIIndexableAssetDependencyPagingHelper(
    ...
    private void _DrawUI() {
      ...
      try {
        ...
        var tabIndex = _DrawHorizontalTabs(new string[] { "Description", string.Format("Dependencies ({0})", _totalAssetDependencies),
          "MetaData", "Tags (0)", "File Data", "Index Data" }, _selectedTabIndex);
        ...
        switch (_selectedTabIndex) {
          case 0:
            _BeginHorizontal();
            _BeginVertical();
            _DrawHorizontalLabelField("Preview Image", 150);
            try {
              var assetPreviewIcontexture = UPAIIconFileRetriever.GetAssetPreviewIcon(_assetDocument);
              if (assetPreviewIcontexture != null) {
                EditorGUI.DrawPreviewTexture(new Rect(20, 65, 128, 128), assetPreviewIcontexture);
              }
            } catch (Exception ex) {
            ...
            _BeginVertical();
            _DrawSpaces(2);
            _DrawHorizontalLabelValueField("Name", _GetDocumentString(
              _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_FILENAME_FIELDNAME)), defaultLabelWidth);
            _DrawHorizontalLabelValueField("Type", _GetDocumentString(
              _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_TYPE_FIELDNAME)), defaultLabelWidth);
            _DrawHorizontalLabelValueField("Path", _GetDocumentString(
              _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_PATH_FIELDNAME)), defaultLabelWidth, 450);
            _DrawHorizontalLabelValueField("GUID", _GetDocumentString(
              _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_GUID_FIELDNAME)), defaultLabelWidth);
            _DrawHorizontalLabelValueField("Dependency Checked", (string.Compare(_GetDocumentString(
              _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_DEPENDENCYMAP_FIELDNAME)), "N/A", true) != 0)
                ? "Yes"
                : "No", defaultLabelWidth);
            _DrawHorizontalLabelValueField("Is Resource", _GetDocumentString(
              _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_PATH_FIELDNAME))
              .Contains("/Resources")
                ? "Yes" 
                : "No", defaultLabelWidth);
            ...
            _DrawSpaces(2);
            _BeginHorizontal();
            var importAsset = GUILayout.Button("Import Asset...", GUILayout.Width(150));
            if (_autoTriggerImport) {
              _autoTriggerImport = false;
              importAsset = true;
            }
            if (importAsset) {              
              _EndVertical();
              ...
              if (IsImporting()) {
                UPAISystem.Debug.LogWarning("An import process is currently running, please wiat for it to finish.");
                return;
              }

              _ImportAsset(_GetDocumentString(
                _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_ID_FIELDNAME)));
              return;
            }
            ...
            if (importPackage) {
              ...
              if (IsImporting()) {
                Debug.LogWarning("An import process is currently running, please wiat for it to finish.");
                return;
              }

              _ImportPackage(_GetDocumentString(
                _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_PACKAGEID_FIELDNAME)));
              return;
            }
            if (_packageViewAllowed) {
              var viewPackage = GUILayout.Button("View Package...", GUILayout.Width(150));
              if (viewPackage) {
                _EndVertical();
                _EndHorizontal();
                ...
                _ViewAssetPackage(_GetDocumentString(
                  _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_PACKAGEID_FIELDNAME)));
                return;
              }
            }
            ...
            break;
          case 1:
            if (string.Compare(_GetDocumentString(
              _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_DEPENDENCYMAP_FIELDNAME)), "N/A", true) == 0 ||
                _assetTypes == null ||
                _assetTypes.Length == 0) {
              _BeginHorizontal();
              _BeginVertical();
              _DrawHorizontalLabelValueField("Dependencies", "No dependencies found for this asset.", defaultLabelWidth, 400);
              _DrawSpaces(95);
              _DrawHorizontalLabelValueField("UPAI Version", UPAISystem.UPAIVersion);
              _EndVertical();
              _EndHorizontal();
              ...
              if (_isSearching || (_fileExtensions != null && _fileExtensions.Length <= 2)) {
                _fileExtensions = null;
              }
              ...
              if (_pagingHelper != null || _pagingHelper.TotalItems > 0 || _contentsToDisplay.Length > 0) {
                ...
                if (_selectedGridIndex != -1 && _forceAssetView) {
                  ...
                  _EndVertical();
                  _currentDocument = _SelectItem(_selectedGridIndex);
                  UPAIAssetDataWindow.CreateAssetDataWindow(_currentDocument, _packageViewAllowed);
                  return;
                }
                _EndHorizontal();
              } else {
          ...
          case 2: // Notes          
            _BeginHorizontal();
            _BeginVertical();
            _scrollPosition_MetaData = _DrawVerticalLabelTextAreaField("MetaData", UPAIMetaDataManager.ToFormattedString(_GetDocumentString(
              _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_METADATA_FIELDNAME))), 
              _scrollPosition_MetaData,
          ...
          case 3: // Tags
            _BeginHorizontal();
            _BeginVertical();
            _scrollPosition_TagData = _DrawVerticalLabelTextAreaField("Tags", "Coming Soon...", _scrollPosition_TagData,
              defaultLabelWidth, 795, 550);
            _DrawSpaces(3);
            _DrawHorizontalLabelValueField("UPAI Version", UPAISystem.UPAIVersion);
            _EndVertical();
          ...
          case 4:
            _BeginHorizontal();
            _BeginVertical();
            _DrawHorizontalLabelValueField("Path", string.Format(@"{{PackageData}}\{0}", _GetDocumentString(
              _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_GUID_FIELDNAME))), defaultLabelWidth, 400);
            _DrawHorizontalLabelValueField("Size", _GetDocumentSize(
              _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_FILESIZE_FIELDNAME)), defaultLabelWidth);
            _DrawHorizontalLabelValueField("Date", _GetDocumentDate(
              _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_FILEDATE_FIELDNAME)), defaultLabelWidth);
            _DrawHorizontalLabelValueField("MD5 Hash", _GetDocumentString(
              _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_FILEMD5HASH_FIELDNAME)), defaultLabelWidth);
            _DrawSpaces(86);
            _DrawHorizontalLabelValueField("UPAI Version", UPAISystem.UPAIVersion);
            _EndVertical();
            _EndHorizontal();
          ...
          case 5:
            ...
            _DrawHorizontalLabelValueField("Document ID", _GetDocumentString(
              _assetDocument.GetFieldable(UPAIDocumentFieldNames.ASSET_ID_FIELDNAME)), defaultLabelWidth, 400);
            _DrawSpaces(95);
            _DrawHorizontalLabelValueField("UPAI Version", UPAISystem.UPAIVersion);
            _EndVertical();
            _EndHorizontal();
    ...
    private void _ImportAsset(string assetId) {
      UPAISystem.Debug.LogDebugMessage("{0}.{1}()", CLASS_NAME, "_ImportAsset");
      ...
      UPAISystem.RefreshSystemData(false, true);

      try {
        if (string.IsNullOrEmpty(assetId))
          throw new ArgumentNullException("AssetID");

        var assetDoc = UPAIIndexSearcher.SearchAssets(UPAISystem.UnityVersion, new Dictionary<string, string>() {
          { UPAIDocumentFieldNames.ASSET_ID_FIELDNAME, assetId },
        }, 1).FirstOrDefault();
        if (assetDoc == null)
          throw new Exception(string.Format("Unable to locate asset document for {0}.", assetId));

        var _assetToImport = new UPAIIndexableAssetInfo(assetDoc);
        if (_assetToImport == null)
          throw new Exception("Unable to create indexable asset info from asset document.");

        var assetTypeDefintion = UPAIAssetDataManager.AssetTypeDefintions.FirstOrDefault(x => x.Key == _assetToImport.AssetType);
        if (assetTypeDefintion.Value == null)
          throw new Exception("Unable to determine asset type of requested asset.");

        var _assetTypeDefintion = assetTypeDefintion.Value;

        var assetName = _assetToImport.AssetFileName.Substring(0, _assetToImport.AssetFileName.IndexOf("."));
        var searchString = string.Format("{0}{1}", assetName, 
          (!string.IsNullOrEmpty(_assetTypeDefintion.SearchFilter))
          ? " " + _assetTypeDefintion.SearchFilter
          : "t:Object");

        UPAISystem.Debug.LogFormat("Search: {0}", searchString);

        // Check for current asset in project
        var locatedAssets = AssetDatabase.FindAssets(searchString);
        if (locatedAssets == null || locatedAssets.Length > 0)
          if (locatedAssets.Contains(_assetToImport.AssetGuid)) {                       
            var _importedAssetPath = AssetDatabase.GUIDToAssetPath(_assetToImport.AssetGuid);
            if (!string.IsNullOrEmpty(_importedAssetPath)) {              
              var currentObjectToPing = AssetDatabase.LoadAssetAtPath(_importedAssetPath,
                (_assetTypeDefintion.Type != null)
                  ? _assetTypeDefintion.Type
                  : typeof(UnityEngine.Object));
              if (currentObjectToPing != null) {
                _uiNotificationProcessor.DisplayDialog("Duplicate Asset Import", string.Format("The selected asset appears to already be in the current project, under {0}", _importedAssetPath));
                _uiNotificationProcessor.PingAsset(_assetToImport.AssetGuid, 
                  (_assetTypeDefintion.Type != null)
                    ? _assetTypeDefintion.Type
                    : typeof(UnityEngine.Object));                
                return;
          ...
          if (_assetTypeDefintion != null && (
            _assetTypeDefintion.AssetType == UPAIAssetTypeEnum.AssemblyFile ||
            _assetTypeDefintion.AssetType == UPAIAssetTypeEnum.Script ||
            _assetTypeDefintion.AssetType == UPAIAssetTypeEnum.Plugin ||
            _assetTypeDefintion.AssetType == UPAIAssetTypeEnum.Prefab ||
            _assetTypeDefintion.AssetType == UPAIAssetTypeEnum.Scene)) {
            EditorUtility.DisplayDialog("Script File Import Warning...",
              ...
              "and will cause the UPAI System to close all active UPAI windows." +
          ...
          UPAISystem.ImportAsset(_assetToImport);
      } catch (Exception ex) {
        UPAISystem.Debug.LogError(ex.Message);
      }
    }
    ...
    void _Reset() {
      ...
      if (selectedAssetType == UPAIAssetTypeEnum.Unknown)
        return;
    ...
    private void UPAISystem_AssetImportProcessStarting(object sender, UPAIProcessStartingEventArgs e) {
      _uiNotificationProcessor.ProcessUINotification(e);
    }

    private void UPAISystem_AssetImportProcessStarted(object sender, UPAIProcessStartedEventArgs e) {
      _uiNotificationProcessor.ProcessUINotification(e);     
    }

    private void UPAISystem_AssetImportProcessCompleting(object sender, UPAIProcessCompletingEventArgs e) {
      _uiNotificationProcessor.ProcessUINotification(e);
    }

    private void UPAISystem_AssetImportProcessCompleted(object sender, UPAIProcessCompletedEventArgs e) {
      try {
        UPAIUnityEditorRecoveryManager.SetLastAssetIdImported(_assetDocument.GetField(UPAIDocumentFieldNames.ASSET_ID_FIELDNAME).StringValue);
      ...
      _uiNotificationProcessor.ProcessUINotification(e);
      _uiNotificationProcessor.RefreshAssetDatabase();
      _uiNotificationProcessor.PingAsset(_assetToImport.AssetGuid, (_assetTypeDefintion != null && _assetTypeDefintion.Type != null)
        ? _assetTypeDefintion.Type
        : typeof(UnityEngine.Object));
    }

    private void UPAISystem_AssetImportProcessAborting(object sender, UPAIProcessAbortingEventArgs e) {
      _uiNotificationProcessor.ProcessUINotification(e);
    }

    private void UPAISystem_AssetImportProcessAborted(object sender, UPAIProcessAbortedEventArgs e) {
      _uiNotificationProcessor.ProcessUINotification(e);
    }
